...
Below we will investigate how to save the output of an **R** plot and label it in a way similar to that done above.  Recall the `flights` dataset from Chapter \@ref(rmd-basics).  (Note that we've shown a different way to reference a section or chapter here.)  We will next explore a bar graph with the mean flight departure delays by airline from Portland for 2014.  Note also the use of the `scale` parameter which is discussed on the next page. Figure \@ref(fig:delaysboxplot) appears on its own page in landscape orientation, not for any good reason, but to test that the page number will still appear in the right place (the bottom right corner, so that if you flip the page back to portrait, the position and orientation of the page number will be the same on portrait and landscape pages).
